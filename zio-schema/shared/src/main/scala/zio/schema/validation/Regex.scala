...
sealed trait Regex { self =>
  private lazy val regexString = Regex.toRegexString(self)
  private lazy val compiledRegex
    : Pattern = Pattern.compile(regexString) // Scala regex isn't compiling, using Java directly
  ...
  def test(string: String): Boolean =
    compiledRegex.matcher(string).matches()
}
