...
sealed trait CaseSet { self =>
  ...
  type Accessors[Whole, Lens[_, _, _], Prism[_, _], Traversal[_, _]]
...
object CaseSet {
  ...
  final case class Empty[Z]() extends CaseSet { self =>
    ...
    override type Accessors[Whole, Lens[_, _, _], Prism[_, _], Traversal[_, _]] = Unit
  ...
  final case class Cons[A, +T <: CaseSet.Aux[Z], Z](head: Case[A, Z], tail: T) extends :+:[A, T] { self =>
    ...
    override type Accessors[Whole, Lens[_, _, _], Prism[_, _], Traversal[_, _]] =
