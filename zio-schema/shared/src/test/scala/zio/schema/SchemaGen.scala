...
object SchemaGen {
  ...
  val anyEnumeration: Gen[Random with Sized, Schema[Map[String, _]]] =
    Gen.mapOf(Gen.anyString, anySchema).map(Schema.enumeration)

  type EnumerationAndGen = (Schema[Map[String, _]], Gen[Random with Sized, Map[String, _]])
  ...
  type EnumerationAndValue = (Schema[Map[String, _]], Map[String, _])
  ...
  val anyRecord: Gen[Random with Sized, Schema[Map[String, _]]] =
    Gen.mapOf(Gen.anyString, anySchema).map(Schema.record)

  type RecordAndGen = (Schema[Map[String, _]], Gen[Random with Sized, Map[String, _]])
  ...
  type RecordAndValue = (Schema[Map[String, _]], Map[String, _])
  ...
  def transformRecord[A](schema: Schema[Map[String, _]]): RecordTransform[A] =
  ...
  def transformEnumeration[A](schema: Schema[Map[String, _]]): EnumerationTransform[_] =
    Schema.Transform[Map[String, _], A](schema, _ => Left("Not implemented."), _ => Left("Not implemented."))
