...
class TestAccessorBuilder extends AccessorBuilder {
  ...
  override type Traversal[S[_], A] = TestAccessorBuilder.Traversal[S, A]
  ...
  override def makeTraversal[S[_], A](collection: Schema.Sequence[S, A], element: Schema[A]): Traversal[S, A] =
...
object TestAccessorBuilder {
  ...
  case class Traversal[S[_], A](s: Schema.Sequence[S, A], a: Schema[A])
}
