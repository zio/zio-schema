/*
 * Copyright 2023-2024 ZIO Schema
 *
...
/**
 * ZIO Schema Codec for Apache Thrift.
 * Migrated to support ZIO 2.x runtime and streaming architecture.
 */
object ThriftCodec {
  ...
  final class Encoder extends MutableSchemaBasedValueProcessor[Unit, Encoder.Context] {
    ...
    private def fail(message: String): Unit = sys.error(message)
  ...
  final class Decoder(chunk: Chunk[Byte]) extends MutableSchemaBasedValueBuilder[Any, DecoderContext] {
    ...
    def decodePrimitive[A](f: TProtocol => A, name: String): PrimitiveDecoder[A] =
      path =>
        Try(f(p)).fold(
          _ => throw MalformedFieldWithPath(path, s"Unable to decode $name"),
          identity
        )
    ...
    override protected def createRecord(
      ...
      val allValues = record.fields.zipWithIndex.map {
        case (field, idx) =>
