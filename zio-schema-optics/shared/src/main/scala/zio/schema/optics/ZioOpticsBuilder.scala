    ...
    product.toDynamic(whole) match {
      case DynamicValue.Record(_, values) =>
        values
          .get(term.name)
          .map { dynamicField =>
            term.schema.fromDynamic(dynamicField).toEither match {
              case Left(error)  => Left(OpticFailure(error.toString()) -> whole)
  ...
  private[optics] def makeLensSet[S, A](
    ...
    product.toDynamic(whole) match {
      case DynamicValue.Record(name, values) =>
        ...
        product.fromDynamic(DynamicValue.Record(name, updated)).toEither match {
          case Left(error)  => Left(OpticFailure(error.toString()) -> whole)
          case Right(value) => Right(value)
        }
      case _ => Left(OpticFailure(s"Unexpected dynamic value for whole") -> whole)
