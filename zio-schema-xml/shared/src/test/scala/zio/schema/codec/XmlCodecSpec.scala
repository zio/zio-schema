...
suite("empty collections config")(
  test("list empty") {
    val cfg = XmlCodec.Config.default.copy(prettyPrint = false, ignoreEmptyCollections = true, explicitNulls = false)
    val xmlStr = XmlCodec
      .xmlEncoder(cfg)(Schema[ListAndMapAndOption])
      .encodeXml(ListAndMapAndOption(Nil, Map("foo" -> 1), Some("foo")))
      .toString()
    val expected = "<record><map><map><entry><key>foo</key><value><int>1</int></value></entry></map></map><option><string>foo</string></option></record>"
    assert(normalize(xmlStr))(equalTo(normalize(expected)))
  },
  test("map empty") {
    val cfg = XmlCodec.Config.default.copy(prettyPrint = false, ignoreEmptyCollections = true, explicitNulls = false)
    val xmlStr = XmlCodec
      ...
      .toString()
    val expected = "<record><list><seq><string>foo</string></seq></list><option><string>foo</string></option></record>"
    assert(normalize(xmlStr))(equalTo(normalize(expected)))
  },
  test("option empty") {
    val cfg = XmlCodec.Config.default.copy(prettyPrint = false, ignoreEmptyCollections = true, explicitNulls = false)
    val xmlStr = XmlCodec
      ...
      .toString()
    val expected = "<record><list><seq><string>foo</string></seq></list><map><map><entry><key>foo</key><value><int>1</int></value></entry></map></map></record>"
    assert(normalize(xmlStr))(equalTo(normalize(expected)))
  },
  test("all empty") {
    val cfg = XmlCodec.Config.default.copy(prettyPrint = false, ignoreEmptyCollections = true, explicitNulls = false)
  ...
  test("all empty, but don't ignore empty collections") {
    val cfg = XmlCodec.Config.default.copy(prettyPrint = false, ignoreEmptyCollections = false, explicitNulls = true)
    val xmlStr = XmlCodec
      .xmlEncoder(cfg)(Schema[ListAndMapAndOption])
      .encodeXml(ListAndMapAndOption(Nil, Map.empty, None))
      .toString()
...
// Suite for records
 ...
 test("missing fields should be replaced by default values") {
  ...
  test("of primitives") {
    ...
    val xmlStr = XmlCodec.xmlEncoder(cfg)(schema).encodeXml(
  ...
  check(searchRequestGen) { searchRequest =>
    ...
    val expectedXml: String =
      XmlCodec.xmlEncoder(XmlCodec.Config.default)(searchRequestSchema)
...
,
 ...
 suite("enumeration")(
  test("of primitives") {
    ...
    val xmlStr = XmlCodec.xmlEncoder(cfg)(schema).encodeXml(input).toString()
    val expected =
      """<enum type="string"><string>foo</string></enum>"""
  ...
  test("pretty printing with discriminator field") {
    val cfg = XmlCodec.Config.default.copy(prettyPrint = true)
    val xml = XmlCodec.xmlEncoder(cfg)(Schema[OneOf4])
  ...
  test("pretty printing with discriminator key") {
    val cfg = XmlCodec.Config.default.copy(prettyPrint = true)
    val xml = XmlCodec.xmlEncoder(cfg)(Schema[OneOf])
...
suite("dynamic direct mapping")(
 ...
 suite("zio.schema.codec.Xml encoding")(
  test("Xml.Record") {
  val cfg    = XmlCodec.Config.default.copy(prettyPrint = false)
  val xmlStr = XmlCodec.xmlEncoder(cfg)(recordSchema).encodeXml(
    ListMap[String, Any]("foo" -> "s", "bar" -> 1)
  ).toString()
  val expected =
    """<record>
...
test("Xml.Arr") {
  ...
  val xmlStr = XmlCodec.xmlEncoder(cfg)(arrSchema).encodeXml(
 ...
 suite("Streams")(
  ...
  val cfg    = XmlCodec.Config.default.copy(prettyPrint = false)
  val xmlStr = XmlCodec.xmlEncoder(cfg)(Schema.list(Schema[Boolean])).encodeXml(List(true, true, false)).toString()
  val expected =
  ...
  val cfg    = XmlCodec.Config.default.copy(prettyPrint = false)
  val xmlStr = XmlCodec.xmlEncoder(cfg)(Schema.list(personSchema))
  ...
  test("with option fields encoded as null") {
    ...
    val xmlStr =
      """<record>
        ...
        |  <bar></bar>
        |</record>""".stripMargin.replaceAll("\n", "").replaceAll(">\\s+<", "><")
    assertDecodes(recordWithOptionSchema, ListMap("foo" -> Some("s"), "bar" -> None), charSequenceToByteChunk(xmlStr), cfg)
  },
  ...
  test("case class with int option field present (at end) from pretty printed json") {
    ...
    val xmlStr =
      """<record>
        ...
        |  <b><int>1</int></b>
        |</record>""".stripMargin.replaceAll("\n", "").replaceAll(">\\s+<", "><")
    assertDecodes(WithOptionFields.schema, WithOptionFields(Some("s"), Some(1)), charSequenceToByteChunk(xmlStr), cfg)
  },
  test("case class with option fields omitted when empty") {
    val cfg = XmlCodec.Config.default.copy(prettyPrint = false)
    val xmlStr =
...
suite("any schema")(
  ...
  test("with caseName") {
    ...
    assertDecodesToError(Schema[Color], ("<color>Grass</color>"), List(XmlError.Raw("zio.schema.codec.DecodeError$ReadError: Unrecognized enum case: Grass")))
  },
  ...
  test("invalid case") {
    assertDecodesToError(Schema[Color], ("<color>not a color</color>"), List(XmlError.Raw("zio.schema.codec.DecodeError$ReadError: Unrecognized enum case: not a color")))
  }
   ),
  ...
  test("with caseName") {
    ...
    assertDecodesToError(Schema[BigEnum], ("<bigEnum>Case00</bigEnum>"), List(XmlError.Raw("zio.schema.codec.DecodeError$ReadError: Unrecognized enum case: Case00")))
  },
  test("with caseAliases") {
    assertEncodesThenDecodes(Schema[BigEnum], BigEnum.Case00) &>
  ...
  test("invalid case") {
    assertDecodesToError(Schema[BigEnum], ("<bigEnum>CaseXX</bigEnum>"), List(XmlError.Raw("zio.schema.codec.DecodeError$ReadError: Unrecognized enum case: CaseXX")))
  }
 ),
