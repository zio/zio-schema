...
object DeriveSchemaSpec extends DefaultRunnableSpec {
  ...
  override def spec: ZSpec[Environment, Failure] = suite("DeriveSchemaSpec")(
    test("DeriveSchema correctly derives schema for UserId case class") {
      ...
      val userIdSchema: Schema[UserId] = DeriveSchema.gen[UserId]

      val expectedSchema: Schema[UserId] = Schema.Transform[Map[String, String], UserId](
        Schema.Record(Map("id" -> Schema.Primitive(StandardType.StringType))),
        map => Right(UserId(map("id"))), // ignored in equality comparison
    ...
    test("DeriveSchema correctly derives schema for complex ADT") {
      ...
      val expectedSchema: Schema[Status] =
        Schema.Transform[Map[String, _], Status](
          Schema.Enumeration(
            Map(
              "Ok" -> Schema.Transform[Map[String, _], Ok](
                Schema.Record(
                  Map(
              ...
              "Failed" -> Schema.Transform[Map[String, _], Failed](
                Schema.Record(
                  Map(
              ...
              "Pending" -> Schema.Transform[Map[String, _], Pending.type](
                Schema.Record(Map()),
