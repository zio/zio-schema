...
object DeriveSchema {
  ...
  def genImpl[T: c.WeakTypeTag](c: whitebox.Context): c.Tree = {
    ...
    def concreteType(seenFrom: Type, tpe: Type): Type = tpe.asSeenFrom(seenFrom, seenFrom.typeSymbol.asClass)
    ...
    def deriveRecord(tpe: Type, stack: List[Frame[c.type]]): Tree =
