...
trait Deriver[F[_]] {
  ...
  def deriveTupleN[T](schemasAndInstances: => Chunk[(Schema[_], F[_])], summoned: => Option[F[T]]): F[T] = {
    ...
    val recordSchema = Deriver.tupleToRecordSchema[F, T](arity, schemasAndInstances)
    deriveRecord(
      recordSchema,
...
object Deriver {
  ...
  trait AutoAcceptSummoned[F[_]] extends Deriver[F] {
    def deriveRecord[A](record: Schema.Record[A], fields: => Chunk[F[_]]): F[A]
    def deriveEnum[A](`enum`: Schema.Enum[A], cases: => Chunk[F[_]]): F[A]
    ...
    def deriveEither[A, B](either: Schema.Either[A, B], left: => F[A], right: => F[B]): F[Either[A, B]] =
      deriveEnum(either.toEnum, Chunk(left, right))
    ...
    def deriveTuple2[A, B](tuple: Schema.Tuple2[A, B], left: => F[A], right: => F[B]): F[(A, B)] =
      deriveTupleN[(A, B)](Chunk(tuple.left -> left, tuple.right -> right))
    ...
    def deriveTupleN[T](schemasAndInstances: => Chunk[(Schema[_], F[_])]): F[T] = {
      val arity        = schemasAndInstances.length
      val recordSchema = Deriver.tupleToRecordSchema[F, T](arity, schemasAndInstances)
      deriveRecord(
        recordSchema,
    ...
    final override def deriveEnum[A](`enum`: Schema.Enum[A], cases: => Chunk[F[_]], summoned: => Option[F[A]]): F[A] =
      summoned.getOrElse {
    ...
    final override def deriveTupleN[T](
      schemasAndInstances: => Chunk[(Schema[_], F[_])],
      summoned: => Option[F[T]]
    ): F[T] =
      summoned.getOrElse {
  ...
  private def tupleToRecordSchema[F[_], T](
    arity: Int,
    schemasAndInstances: => Chunk[(Schema[_], F[_])]
  ): Schema.Record[T] =
    arity match {
