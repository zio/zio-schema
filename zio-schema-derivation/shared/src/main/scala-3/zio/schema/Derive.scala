...
object Derive {
  ...
  inline def derive[F[_], A](deriver: Deriver[F])(implicit schema: Schema[A]): F[A] = ${
    deriveInstance[F, A]('deriver, 'schema)
  }

  private def deriveInstance[F[_]: Type, A: Type](deriver: Expr[Deriver[F]], schema: Expr[Schema[A]])(using
    ctx: Quotes
  ): Expr[F[A]] =
...
private case class DeriveInstance()(using val ctx: Quotes) {
  ...
  def deriveInstance[F[_]: Type, A: Type](
    deriver: Expr[Deriver[F]],
            ...
            if (typeRef.isOpaqueAlias) {
                  ...
                  Expr.summon[StandardType[ut]] match {
                    ...
                    case None =>
                      report.errorAndAbort(
                        s"Opaque type ${typeRepr.show} is not supported because its underlying type is not a primitive type"
                      )
                  }
              }
            } else {
              typeRepr.asType match {
                ...
                case '[scala.collection.immutable.Set[a]] =>
                  val (schemaRef, schemaRefExpr) =
                    createSchemaRef[scala.collection.immutable.Set[a], Schema.Set[a]](stack)
                ...
                case '[Vector[a]] =>
                  val (schemaRef, schemaRefExpr) =
                    createSchemaRef[scala.collection.immutable.Vector[a], Schema.Sequence[Vector[a], a, _]](stack)
                ...
                case '[scala.collection.immutable.Map[a, b]] =>
                  val (schemaRef, schemaRefExpr) =
                    createSchemaRef[scala.collection.immutable.Map[a, b], Schema.Map[a, b]](stack)
                          ...
                          typeRepr.asType match {
                            case '[(a, b)] =>
                              ...
                              lazyVals[F[A]](
                                ...
                                selfRef   -> '{
                                  $deriver.deriveTupleN[(a, b)](
                                    Chunk(
                                      $schemaRefExpr.left  -> Deriver.wrap($instanceA),
                                      $schemaRefExpr.right -> Deriver.wrap($instanceB)
                                    ),
                                    $summoned
                                  )
                                }
                            ...
                            case '[(t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15)] =>
                              tupleN[F, (t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15)](
                                ...
                                deriver,
                                schema
                                  .asExprOf[Schema[(t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12, t13, t14, t15)]],
                                stack,
                                Chunk(
                                  TypeRepr.of[t1],
                                  TypeRepr.of[t2],
  ...
  def deriveCaseObject[F[_]: Type, A: Type](top: Boolean, deriver: Expr[Deriver[F]], schema: Expr[Schema[A]])(using
    Quotes
  ): Expr[F[A]] = {
    val summoned = summonOptionalIfNotTop[F, A](top)
    Expr.summon[scala.reflect.ClassTag[A]] match {
  ...
  def deriveCaseClass[F[_]: Type, A: Type](
    top: Boolean,
    mirror: Mirror,
    stack: Stack,
    deriver: Expr[Deriver[F]],
    ...
    if (labels.length <= 22) {
      ...
      lazyVals[F[A]](
        ...
        selfRef         -> '{
          $deriver.deriveTransformedRecord[ListMap[String, _], A](
            $recordSchemaRefExpr,
            $schemaRefExpr,
            Chunk(${ Varargs(fieldInstances) }: _*),
            $summoned
          )
        }
      )
  ...
  def deriveEnum[F[_]: Type, A: Type](
    top: Boolean,
    mirror: Mirror,
    stack: Stack,
    deriver: Expr[Deriver[F]],
  ...
  def createSchemaRef[T: Type, S <: Schema[T]: Type](stack: Stack)(using Quotes): (Ref, Expr[S]) = {
    val schemaSymbol =
      Symbol.newVal(Symbol.spliceOwner, s"schema${stack.size}", TypeRepr.of[S], Flags.Lazy, Symbol.spliceOwner)
  ...
  def tupleN[F[_]: Type, A: Type](
    ...
    val nestedTupleType = tpes.tail.foldLeft(tpes.head) { case (xs, x) =>
      TypeRepr.of[scala.Tuple2].appliedTo(List(xs, x))
    }
