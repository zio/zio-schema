...
object AvroSchemaCodec extends AvroSchemaCodec {
  ...
  def decode: Chunk[Byte] => Either[String, Schema[_]] = {
    ...
    bytes => {
      ...
      avroSchema.flatMap(toZioSchema(_))
    }
  }

  def decodeFromApacheAvro: SchemaAvro => Either[String, Schema[_]] = toZioSchema(_)
  ...
  private def toZioSchema(avroSchema: SchemaAvro): Either[String, Schema[_]] =
    for {
      ...
      _ <- Try { LogicalTypes.fromSchema(avroSchema) }.toEither.left.map(e => e.getMessage)
      result <- avroSchema.getType match {
                 case SchemaAvro.Type.RECORD =>
                   RecordType.fromAvroRecord(avroSchema) match {
                     ...
                     case Some(RecordType.Duration) =>
                       val chronoUnit =
                         DurationChronoUnit.fromAvroDuration(avroSchema).getOrElse(DurationChronoUnit.default)
                       Right(Schema.primitive(StandardType.Duration(chronoUnit.chronoUnit)))
                 ...
                 case SchemaAvro.Type.FIXED =>
                   val fixed = if (avroSchema.getLogicalType == null) {
                     ...
                     // TODO: Java implementation of Apache Avro does not support logical type Duration yet: AVRO-2123 with PR https://github.com/apache/avro/pull/1263
                 ...
                 case SchemaAvro.Type.INT =>
                   IntType.fromAvroInt(avroSchema) match {
                     ...
                     case None =>
                       ...
                       } else if (avroSchema.getLogicalType.isInstanceOf[LogicalTypes.TimeMillis]) {
                       ...
                       } else if (avroSchema.getLogicalType.isInstanceOf[LogicalTypes.Date]) {
                         val formatter = Formatter.fromAvroStringOrDefault(avroSchema, avroSchema.getLogicalType)
                         formatter.map(
                           formatter => Schema.primitive(StandardType.LocalDateType(formatter.dateTimeFormatter))
                         )
                       } else Left(s"Unsupported int logical type ${avroSchema.getLogicalType.getName}")
                   }
                 case SchemaAvro.Type.LONG =>
                   if (avroSchema.getLogicalType == null) {
                     Right(Schema.primitive(StandardType.LongType))
                   } else if (avroSchema.getLogicalType.isInstanceOf[LogicalTypes.TimeMicros]) {
                     val formatter = Formatter.fromAvroStringOrDefault(avroSchema, avroSchema.getLogicalType)
                     formatter.map(
                       formatter => Schema.primitive(StandardType.LocalTimeType(formatter.dateTimeFormatter))
                     )
                   } else if (avroSchema.getLogicalType.isInstanceOf[LogicalTypes.TimestampMillis]) {
                     val formatter = Formatter.fromAvroStringOrDefault(avroSchema, avroSchema.getLogicalType)
                     formatter.map(formatter => Schema.primitive(StandardType.InstantType(formatter.dateTimeFormatter)))
                   } else if (avroSchema.getLogicalType.isInstanceOf[LogicalTypes.TimestampMicros]) {
                     val formatter = Formatter.fromAvroStringOrDefault(avroSchema, avroSchema.getLogicalType)
                     formatter.map(formatter => Schema.primitive(StandardType.InstantType(formatter.dateTimeFormatter)))
                   } else if (avroSchema.getLogicalType.isInstanceOf[LogicalTypes.LocalTimestampMillis]) {
                     val formatter = Formatter.fromAvroStringOrDefault(avroSchema, avroSchema.getLogicalType)
                     formatter.map(
                       formatter => Schema.primitive(StandardType.LocalDateTimeType(formatter.dateTimeFormatter))
                     )
                   } else if (avroSchema.getLogicalType.isInstanceOf[LogicalTypes.LocalTimestampMicros]) {
                     val formatter = Formatter.fromAvroStringOrDefault(avroSchema, avroSchema.getLogicalType)
                     formatter.map(
                       formatter => Schema.primitive(StandardType.LocalDateTimeType(formatter.dateTimeFormatter))
                     )
                   } else Left(s"Unsupported long logical type ${avroSchema.getLogicalType.getName}")
  ...
  private def toAvroSchema(schema: Schema[_]): Either[String, SchemaAvro] = {
    Try {
      schema match {
        ...
        case seq: Schema.Sequence[_, _, _] => toAvroSchema(seq.schemaA).map(SchemaAvro.createArray(_))
        case set: Schema.SetSchema[_]      => toAvroSchema(set.as).map(SchemaAvro.createArray(_))
        ...
        case Primitive(standardType, _) =>
          standardType match {
            ...
            case StandardType.MonthDayType =>
              toAvroSchema(monthDayStructure).map(_.addMarkerProp(RecordDiscriminator(RecordType.MonthDay)))
  ...
  private[codec] def toAvroEnum(`enum`: Enum[_]): Either[String, SchemaAvro] = {
    ...
    if (isAvroEnumEquivalent) {
      for {
        name            <- getName(`enum`)
        doc             = getDoc(`enum`.annotations).getOrElse(null)
        ...
        result = SchemaAvro.createEnum(name, doc, namespaceOption.getOrElse(null), symbols)
      } yield result
    } else {
  ...
  private[codec] def toAvroRecord(record: Record[_]): Either[String, SchemaAvro] =
    for {
      ...
      result <- record.structure.map(toAvroRecordField(_)).toList.partitionMap(identity) match {
                 case (Nil, right) =>
                   Right(
                     SchemaAvro.createRecord(
                       name,
                       getDoc(record.annotations).getOrElse(null),
                       namespaceOption.getOrElse(null),
  ...
  private[codec] def toAvroMap(map: MapSchema[_, _]): Either[String, SchemaAvro] =
    map.ks match {
      case p: Schema.Primitive[_] if p.standardType == StandardType.StringType =>
        toAvroSchema(map.vs).map(SchemaAvro.createMap(_))
      case _ =>
        val tupleSchema = Schema
          .Tuple(map.ks, map.vs)
          .annotate(AvroAnnotations.name("Tuple"))
          .annotate(AvroAnnotations.namespace("scala"))
        toAvroSchema(tupleSchema).map(SchemaAvro.createArray(_))
    }
    ...
    val baseAvroType = schema.annotations.collectFirst { case AvroAnnotations.decimal(decimalType) => decimalType }
      ...
      case DecimalType.Fixed(size) =>
        for {
          ...
          doc             = getDoc(schema.annotations).getOrElse(null)
          result          = SchemaAvro.createFixed(name, doc, namespaceOption.getOrElse(null), size)
        } yield result
    ...
    toAvroSchema(value.schema).map(
      schema =>
        new SchemaAvro.Field(
          ...
          schema,
          getDoc(value.annotations).getOrElse(null),
          getDefault(value.annotations).getOrElse(null),
  ...
  private[codec] def getName(schema: Schema[_]): Either[String, String] =
    schema.annotations.collectFirst { case AvroAnnotations.name(name) => name } match {
      ...
      case None =>
        Right(s"hashed_${schema.ast.toString.hashCode().toString.replaceFirst("-", "n")}") // TODO: better way to generate a (maybe stable) name?
    }
  ...
  private[codec] def toZioEnumeration[A, Z](avroSchema: SchemaAvro): Either[String, Schema[Z]] = {
    ...
    val caseSet = cases.toList.partitionMap(identity) match {
      case (Nil, right) =>
        Try { CaseSet(right: _*).asInstanceOf[CaseSet { type EnumType = Z }] }.toEither.left.map(_.getMessage)
      case (left, _) => Left(left.mkString("\n"))
    }
    caseSet.map(Schema.enumeration(_))
  }
  ...
  private[codec] def toZioRecord(avroSchema: SchemaAvro): Either[String, Schema[_]] =
    if (avroSchema.getObjectProp(UnionWrapper.propName) != null) {
      ...
      val fields: Either[String, List[Field[_]]] = {
        avroSchema.getFields.asScala.map(toZioField(_)).toList.partitionMap(identity) match {
  ...
  private[codec] def buildZioAnnotations(schema: SchemaAvro): Chunk[StaticAnnotation] = {
    ...
    val namespace = Try { Option(schema.getNamespace).map(AvroAnnotations.namespace(_)) }.toOption.flatten
    ...
    val error   = Try { Option.when(schema.isError)(AvroAnnotations.error) }.toOption.flatten
    val default = Try { Option.when(schema.getEnumDefault != null)(AvroAnnotations.default(schema.getEnumDefault)) }.toOption.flatten
  ...
  private[codec] def buildZioAnnotations(field: SchemaAvro.Field): Chunk[Any] = {
    ...
    val default           = Try { Option.when(field.hasDefaultValue)(AvroAnnotations.default(field.defaultVal())) }.toOption.flatten
  ...
  private[codec] def toZioStringEnum(avroSchema: SchemaAvro): Either[String, Schema[_]] = {
    val cases =
      avroSchema.getEnumSymbols.asScala.map(s => Schema.Case[String, String](s, Schema[String], identity(_))).toSeq
