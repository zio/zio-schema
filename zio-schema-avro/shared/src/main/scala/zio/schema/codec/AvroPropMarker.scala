...
object AvroPropMarker {
  ...
  object Formatter {
    // TODO: use only a single property marker and distinguish by value. A json with "zio.schema.codec.avro.dateTimeFormatter": false is confusing that is still uses the dateTimeFormatter regardless of the value
    ...
    private def getDefaultByLogicalType(logicalType: LogicalType): Formatter =
      if (logicalType.isInstanceOf[LogicalTypes.Date]) {
        Formatter(DateTimeFormatter.ISO_DATE)
      } else if (logicalType.isInstanceOf[LogicalTypes.LocalTimestampMicros]) {
        Formatter(DateTimeFormatter.ISO_LOCAL_DATE_TIME)
      } else if (logicalType.isInstanceOf[LogicalTypes.LocalTimestampMillis]) {
        Formatter(DateTimeFormatter.ISO_LOCAL_DATE_TIME)
      } else if (logicalType.isInstanceOf[LogicalTypes.TimeMicros]) {
        Formatter(DateTimeFormatter.ISO_LOCAL_TIME)
      } else if (logicalType.isInstanceOf[LogicalTypes.TimeMillis]) {
        Formatter(DateTimeFormatter.ISO_LOCAL_TIME)
      } else if (logicalType.isInstanceOf[LogicalTypes.TimestampMicros]) {
        Formatter(DateTimeFormatter.ISO_INSTANT)
      } else if (logicalType.isInstanceOf[LogicalTypes.TimestampMillis]) {
        Formatter(DateTimeFormatter.ISO_INSTANT)
      } else {
        Formatter(DateTimeFormatter.ISO_INSTANT)
      }
