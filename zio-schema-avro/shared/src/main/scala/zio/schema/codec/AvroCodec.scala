...
object AvroCodec extends Codec {
  ...
  object Encoder {
    ...
    def encode[A](schema: Schema[A], value: A): Chunk[Byte] = schema match {
      ...
      case e: Schema.Enum[_]    => encodeEnum(e, value)
      case r: Schema.Record[_]  => encodeRecord(r, value)
      case _: Schema.Fail[_]    => Chunk.empty
    }

    private def encodeInt(i: Int): Chunk[Byte] = {
      val n = (i << 1) ^ (i >> 31);
      if (n > 0x7F) {
        ((n | 0x80) & 0xFF).byteValue() +: encodeInt(n >>> 7)
      } else {
        Chunk.empty
      }
    }

    private def encodeLong(l: Long): Chunk[Byte] = {
      val n = (l << 1) ^ (l >> 63);
      if (n > 0x7F) {
        ((n | 0x80) & 0xFF).byteValue() +: encodeLong(n >>> 7)
      } else {
        Chunk.empty
      }
    }
    ...
    def encodeFloat(value: Float): Chunk[Byte] = {
      val intBits: Int = lang.Float.floatToIntBits(value)
    ...
    def encodeDouble(value: Double): Chunk[Byte] = {
      val longBits: Long = lang.Double.doubleToLongBits(value)
      val bytes: Array[Byte] =
    ...
    private def encodePrimitive[A](standardType: StandardType[A], v: A): Chunk[Byte] = standardType match {
      ...
      case StandardType.DayOfWeekType  => encodeInt(v.getValue())
      case StandardType.Month          => encodeInt(v.getValue())
    ...
    private def encodeEnum[A](e: Schema.Enum[A], value: A): Chunk[Byte] = {
    ...
    //scalafmt: { maxColumn = 400, optIn.configStyleArguments = false }
    private def encodeRecord[A](r: Schema.Record[A], value: A): Chunk[Byte] = r match {
