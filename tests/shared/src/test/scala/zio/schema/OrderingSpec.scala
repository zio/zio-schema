...
object OrderingSpec extends ZIOSpecDefault {
  ...
  def genOrderedPairEither[A, B](
    lSchema: Schema[A],
    rSchema: Schema[B]
  ): Gen[Sized, (Either[A, B], Either[A, B])] =
    Gen.oneOf(
      for {
  ...
  def genAnyOrderedPairChunks: Gen[Sized, SchemaAndPair[Any]] = anySchema.flatMap(genOrderedPairChunk(_))
  ...
  def genOrderedPair[A](schema: Schema[A]): Gen[Sized, (A, A)] =
    (genFromSchema(schema) <*> genFromSchema(schema)).withFilter { case (l, r) =>
